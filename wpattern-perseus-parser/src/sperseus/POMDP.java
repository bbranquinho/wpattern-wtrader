...
public class POMDP implements Serializable {
	...
	public double[] getRewFnTabular(int actId) {
		return OP.convert2array(actions[actId].rewFn, concatenateArray(
				varIndices, primeVarIndices));
	}
	...
	public double[] getTransFnTabular(int actId) {
		...
		return OP.convert2array(fullTF, concatenateArray(varIndices,
				primeVarIndices));
	}
	...
	public double[] getObsFnTabular(int actId, int obsId) {
		...
		return OP.convert2array(actions[actId].obsFn[obsId], concatenateArray(
				primeVarIndices, tmpidarray));
	}
	...
	public void readFromFile(String fileName, boolean debb) {
		...
		for (int i = 0; i < nObsVars; i++) {
			...
			for (int j = 0; j < obsVars[i].arity; j++) {
				obsVars[i].addValName(j, ((String) rawpomdp.valNames.get(
						nStateVars + i).get(j)));
			}
		...
		for (int varId = 0; varId < nStateVars; varId++) {
			initialBelState_f[varId] = OP.addMultVarElim(initialBelState, MySet
					.remove(varIndices, varId + 1));
		}
	...
	public DD beliefUpdate(DD belState, int actId, int[] obsvals) {
		return beliefUpdate(belState, actId, stackArray(primeObsIndices,
				obsvals));
	}
	...
	public DD beliefUpdate(DD belState, int actId, int[][] obsvals) {
		...
		DD nextBelState = OP.addMultVarElim(concatenateArray(belState,
				actions[actId].transFn, restrictedObsFn), varIndices);
		...
		nextBelState = OP.div(nextBelState, OP.addMultVarElim(nextBelState,
				varIndices));
		return nextBelState;
	}
		...
		for (i = 0; i < nObsVars; i++) {
			...
			while (!done) {
				for (int actId = 0; actId < nActions; actId++) {
					...
					tObsFn = OP.addMultVarElim(actions[actId].obsFn, MySet
							.remove(primeObsIndices, primeObsIndices[i]));
	...
	public double[] evaluateObservations(boolean usemax) {
		...
		for (int i = 0; i < nObsVars; i++) {
			...
			for (int actId = 0; actId < nActions; actId++) {
				// eliminate all other observation variables from this
				// observation function
				tObsFn = OP.addMultVarElim(actions[actId].obsFn, MySet.remove(
						primeObsIndices, primeObsIndices[i]));
				...
				for (int b = 0; b < belRegion.length; b++) {
					...
					for (int j = 0; agree && j < obsVars[i].arity; j++) {
						...
						// tObsFn.display();
						// restrictedObsFn[0].display();
						nextBelState[j] = OP.addMultVarElim(concatenateArray(
								belRegion[b], actions[actId].transFn,
								restrictedObsFn), varIndices);
						...
						if (obsProb.getVal() < 1e-8)
							nextBelState[j] = DD.one;
						nextBelState[j] = OP.div(nextBelState[j], OP
								.addMultVarElim(nextBelState[j], varIndices));
						// nextBelState[j].display();
						maxbval[j] = Double.NEGATIVE_INFINITY;
						maxa = 0;
						for (int a = 0; a < alphaVectors.length; a++) {
	...
	public int policyQuery(DD belState, DD[] alphaVectors, int[] policy) {
		...
		System.out.println("num alpha vectors: ");
		System.out.println(alphaVectors.length);
	...
	public double findSimilarFactBelief(DD[] belief, DD[][] belSet, int count,
		...
		for (int i = 0; !done1 & i < count; i++) {
			...
			for (int varId = 0; !done2 & varId < belief.length; varId++) {
				dist = OP.maxAll(OP
						.abs(OP.sub(belSet[i][varId], belief[varId])));
				if (dist > maxnorm) {
					maxnorm = dist;
	...
	public double getSingleValue(DD belState, int varId, int varVal) {
		DD fbs = OP.addMultVarElim(belState, MySet
				.remove(varIndices, varId + 1));
		int vid = fbs.getVar();
		if (vid == 0) {
			// its already a leaf
	...
	public void printBeliefState(DD belState) {
		...
		for (int varId = 0; varId < nStateVars; varId++) {
			fbs[varId] = OP.addMultVarElim(belState, MySet.remove(varIndices,
					varId + 1));
		}
	...
	public void setBelRegionFromData(int maxSize, double threshold, int[][] a,
		...
		for (int epindex = 0; count < maxSize && epindex < a.length; epindex++) {
			...
			for (int stepId = 0; count < maxSize && stepId < a[epindex].length; stepId++) {
				...
				for (int varId = 0; varId < nStateVars; varId++) {
					nextBelState[varId] = OP.addMultVarElim(concatenateArray(
							belState, actions[actId].transFn, restrictedObsFn),
							concatenateArray(MySet.remove(primeVarIndices,
									varId + nVars + 1), varIndices));
	...
	public void solve(int nRounds, int maxSize, int maxTries,
		...
		if (!debug && explorProb < 1.0)
			solveQMDP(15); //changed from no-input by Kyle Morrison
		...
		for (runId = 0; runId < nRuns; runId++) {
			...
			for (stepId = 0; stepId < nSteps; stepId++) {
				...
				nextStateConfig = OP.sampleMultinomial(restrictedTransFn,
						primeVarIndices);
				obsDistn = OP.restrictN(actions[actId].obsFn, concatenateArray(
	...
	public void reachableBelRegionCurrentPolicyMultipleInits(int maxSize,
		...
		for (int i = 0; i < adjunctNames.length; i++) {
			if (adjunctNames[i].startsWith("init")) {
				...
				// here
				for (int varId = 0; varId < nStateVars; varId++) {
					iBelState_f[varId] = OP.addMultVarElim(adjuncts[i], MySet
							.remove(varIndices, varId + 1));
				}
				System.out
	...
	public void reachableBelRegionCurrentPolicy(int maxSize, int maxTries,
		...
		DD[] iBelState_f = new DD[nStateVars];
		for (int varId = 0; varId < nStateVars; varId++) {
			iBelState_f[varId] = OP.addMultVarElim(initialBelState, MySet
					.remove(varIndices, varId + 1));
		}
		...
		while (count < maxSize && numtries < maxTries) {
			...
			for (int stepId = 0; count < maxSize & stepId < episodeLength; stepId++) {
				...
				if (isMDP) {
					...
					obsDist = OP.addMultVarElim(concatenateArray(belState,
							actions[actId].transFn, actions[actId].obsFn),
				...
				for (int varId = 0; varId < nStateVars; varId++) {
					...
					beldiff = OP.maxAll(OP.abs(OP.sub(OP.primeVars(
							nextBelState[varId], -nVars), belState[varId])));
	...
	public void boundedPerseus(int nIterations, int maxAlpha, int firstStep,
		...
		for (int actId = 0; actId < nActions; actId++) {
			...
			for (int i = 0; i < 50; i++) {
				...
				newAlpha = OP.addMultVarElim(concatenateArray(ddDiscFact,
						actions[actId].transFn, newAlpha), primeVarIndices);
	...
	public void boundedPerseusStartFromCurrent(int maxAlpha, int firstStep,
		...
		for (int stepId = firstStep; stepId < firstStep + nSteps; stepId++) {
			...
			maxAbsVal = Math.max(OP.maxabs(concatenateArray(OP
					.maxAllN(alphaVectors), OP.minAllN(alphaVectors))), 1e-10);
	...
	public void displayAlphaVectorSum(DD alphaVector, int varId) {
		DD tempDD = OP.addMultVarElim(alphaVector, MySet.remove(varIndices,
				varId + 1));
		tempDD.display();
	}
	...
	public void displayAlphaVectorPrimeSum(DD alphaVector, int varId) {
		DD tempDD = OP.addMultVarElim(alphaVector, MySet.remove(
				primeVarIndices, varId + nVars + 1));
		tempDD.display();
	}
	...
	public AlphaVector dpBackup(DD[] belState, DD[] primedV, double maxAbsVal) {
		...
		for (int alphaId = 0; alphaId < alphaVectors.length; alphaId++) {
			if (MySet.find(bestObsStrat, alphaId) >= 0) {
				...
				nextValFn = OP.add(nextValFn, OP.multN(concatenateArray(DDleaf
						.myNew(discFact), obsDd, primedV[alphaId])));
			}
		}
		newAlpha = OP.addMultVarElim(
				concatenateArray(actions[bestActId].transFn,
						actions[bestActId].obsFn, nextValFn), concatenateArray(
						primeVarIndices, primeObsIndices));
		newAlpha = OP
	...
	public NextBelState[] oneStepNZPrimeBelStates(DD[] belState,
		...
		for (int actId = 0; actId < nActions; actId++) {
			dd_obsProbs = OP.addMultVarElim(concatenateArray(belState,
					actions[actId].transFn, actions[actId].obsFn),
					concatenateArray(varIndices, primeVarIndices));
			...
			if (!nextBelStates[actId].isempty()) {
				marginals = OP.marginals(concatenateArray(belState,
						actions[actId].transFn, actions[actId].obsFn),
	...
	public class NextBelState {
		...
		public void restrictN(DD[] marginals, int[][] obsConfig) {
			...
			for (int obsPtr = 0; obsPtr < numValidObs; obsPtr++) {
				...
				nextBelStates[obsPtr] = OP.restrictN(marginals, stackArray(
						primeObsIndices, obsConfig[obsId]));
			}
		}
