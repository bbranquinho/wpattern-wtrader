...
public class Solver implements Serializable {
	...
	public static int nIterations = 5; // backup iterations per round (changed from 30 to 10 by Kyle)

	public static int maxAlphaSetSize = 100;

	public static int numBelStates = 100; //changed from 100 by Kyle Morrison for testing

	public static int maxBelStates = 15000; //changed from 10000 for testing
	...
	public static boolean parseArgs(String[] args) {
		...
		if (args.length >= 2) {
			...
			while (numargs > 0 && ioarg.startsWith("-")) {
				// System.out.println("numargs "+numargs+" thearg "+thearg+"
				// ioarg "+ioarg);
				if (ioarg.equals("-i")) {
	...
	/*
	 * will not work if:
	 * 	you have a solution to a problem with the same name but different specifications to your main input problem
	 * 	i.e. you solve coffee.dat, change coffee.dat, and then try to solve it again, all the while leaving checkIfSaved true.
	 * 	better would be solve coffee.dat, change coffee.dat, delete solution, then solve again, or just leave checkIfSaved false
	 */
	public static POMDP solveAndSave(String spuddFileName, boolean checkIfSaved){
		String basename = spuddFileName.substring(0, spuddFileName.lastIndexOf("."));
		iofile = basename + ".pomdp";
		POMDP pomdp = null;
		File solvedPOMDPFile = new File(iofile);
		if (checkIfSaved && solvedPOMDPFile.length() > 0L){ // .length() returns 0L if file does not exist
			...
			try {
				...
				if (obj instanceof POMDP) {
					...
					pomdp = (POMDP) obj;
					pomdp.readFromFile(spuddFileName);
					System.out.println("File was valid, returning object");
				} else{
					System.err.println("File " + iofile + " was not a solution file");
				}
		...
		} else { //compute and save the solution
			pomdp = new POMDP(spuddFileName, false);
			System.out.println("Finished parsing problem file " + spuddFileName);
	...
	public static void generate(String spuddFileName){
		//side effect: saves a file 'iofile' to the directory, 
		//will load this solution on all subsequent attempts to solve the problem
		String basename = spuddFileName.substring(0, spuddFileName.lastIndexOf("."));
		System.out
		...
		POMDP pomdp = solveAndSave(spuddFileName, checkIfSaved);
		...
		if (generate) {
			...
			if (!havepolicy) {
				...
				if (obj instanceof POMDP) {
					 ...
					 * 
					 * DD tmp =
					 * OP.mult(pomdp.actions[0].transFn[1],pomdp.actions[0].transFn[2]);
