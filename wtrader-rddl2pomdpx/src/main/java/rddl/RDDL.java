...
public class RDDL {
	...
	public static boolean ACTION_DEPENDENCY = false;
	...
	public static abstract class LTERM extends EXPR { 
		...
		throws EvalException {
			return sample(subs, s, r, new BooleanPair());
		}
	}
	...
	public static class LVAR extends LTERM {
		...
		public Object sample(HashMap<LVAR, LCONST> subs, State s, Random r, BooleanPair bp)
		throws EvalException {
	...
	public static class TVAR_EXPR extends LTERM {
		...
		public Object sample(HashMap<LVAR,LCONST> subs, State s, Random r, BooleanPair bp) throws EvalException {
			...
			for (int i = 0; i < _alTerms.size(); i++) {
				...
				if (t instanceof LCONST)
					...
					_subTerms.add((ENUM_VAL)tvar.sample(subs, s, r, bp));
				} else
		...
		public void collectGFluents(HashMap<LVAR, LCONST> subs,	State s, HashSet<Pair> gfluents) 
			...
			for (int i = 0; i < _alTerms.size(); i++) {
				...
				if (t instanceof LCONST)
					...
					_subTerms.add((ENUM_VAL)tvar.sample(subs, s, null, new BooleanPair()));
				} else
					throw new EvalException("RDDL.PVAR_EXPR: Unrecognized term " + t + "\n" + this);
			}
	...
	public static abstract class EXPR { 
		...
		public abstract Object sample(HashMap<LVAR,LCONST> subs, State s, Random r, BooleanPair bp) throws EvalException;
	...
	public static class DiracDelta extends EXPR {
		...
		public Object sample(HashMap<LVAR,LCONST> subs, State s, Random r, BooleanPair bp) throws EvalException {
			Object o = _exprRealValue.sample(subs, s, r, bp);
		...
		public EXPR getDist(HashMap<LVAR,LCONST> subs, State s) throws EvalException {
			Double d = ((Number)_exprRealValue.sample(subs, s, null, new BooleanPair())).doubleValue();
	...
	public static class KronDelta extends EXPR {
		...
		public Object sample(HashMap<LVAR,LCONST> subs, State s, Random r, BooleanPair bp) throws EvalException {
			Object o = _exprIntValue.sample(subs, s, r, bp);
			bp.d = true;
		...
		public EXPR getDist(HashMap<LVAR,LCONST> subs, State s) throws EvalException {
 			Object o = _exprIntValue.sample(subs, s, null, new BooleanPair());
	...
	public static class Uniform extends EXPR {
		...
		public Object sample(HashMap<LVAR,LCONST> subs, State s, Random r, BooleanPair bp) throws EvalException {
			try {
				double l = ((Number)_exprLowerReal.sample(subs, s, r, bp)).doubleValue();
				double u = ((Number)_exprUpperReal.sample(subs, s, r, bp)).doubleValue();
				if (l > u)
		...
		public EXPR getDist(HashMap<LVAR,LCONST> subs, State s) throws EvalException {
			...
			try {
				double l = ((Number)_exprLowerReal.sample(subs, s, null, new BooleanPair())).doubleValue();
				double u = ((Number)_exprUpperReal.sample(subs, s, null, new BooleanPair())).doubleValue();
				if (l > u)
					throw new EvalException("RDDL: Uniform upper bound '" + 
							u + "' must be greater than lower bound '" + l + "'");
	...
	public static class Normal extends EXPR {
		...
		public Object sample(HashMap<LVAR,LCONST> subs, State s, Random r, BooleanPair bp) throws EvalException {
			try {
				double mean = ((Number)_normalMeanReal.sample(subs, s, r, bp)).doubleValue();
				double var  = ((Number)_normalVarReal.sample(subs, s, r, bp)).doubleValue();
				if (var < 0)
		...
		public EXPR getDist(HashMap<LVAR,LCONST> subs, State s) throws EvalException {
			double mean = ((Number)_normalMeanReal.sample(subs, s, null, new BooleanPair())).doubleValue();
			double var  = ((Number)_normalVarReal.sample(subs, s, null, new BooleanPair())).doubleValue();
			if (var < 0)
				throw new EvalException("RDDL: Normal variance '" + var +  
						"' must be greater 0");
			// x ~ N(mean,sigma^2) is equivalent to x ~ sigma*N(0,1) + mean
	...
	public static class Exponential extends EXPR {
		...
		public Object sample(HashMap<LVAR,LCONST> subs, State s, Random r, BooleanPair bp) throws EvalException {
			...
			double lambda = ((Number)_exprLambdaReal.sample(subs, s, null, new BooleanPair())).doubleValue();
		...
		public Object sample(HashMap<LVAR,LCONST> subs, State s, Random r, BooleanPair bp) throws EvalException {
			try {
				...
				for (int i = 0; i < _exprProbs.size(); i+=2) {
					...
					enum_probs.add(((Number)((EXPR)_exprProbs.get(i+1)).sample(subs, s, r, bp)).doubleValue());
	...
	public static class Geometric extends EXPR {
		...
		public EXPR getDist(HashMap<LVAR,LCONST> subs, State s) throws EvalException {
			double prob = ((Number)_exprProb.sample(subs, s, null, new BooleanPair())).doubleValue();
	...
	public static class Poisson extends EXPR {
		...
		public EXPR getDist(HashMap<LVAR,LCONST> subs, State s) throws EvalException {
			double lambda = ((Number)_exprLambda.sample(subs, s, null, new BooleanPair())).doubleValue();
	...
	public static class Bernoulli extends BOOL_EXPR {
		...
		public Object sample(HashMap<LVAR,LCONST> subs, State s, Random r, BooleanPair bp) throws EvalException {
			double prob = ((Number)_exprProb.sample(subs, s, r, bp)).doubleValue();
	...
	public static class OPER_EXPR extends EXPR {
		...
		public Object sample(HashMap<LVAR,LCONST> subs, State s, Random r, BooleanPair bp) throws EvalException {
			try {
				Object o1 = _e1.sample(subs, s, r, bp);
				Object o2 = _e2.sample(subs, s, r, bp);
	...
	public static class AGG_EXPR extends EXPR {
		...
		public Object sample(HashMap<LVAR,LCONST> subs, State s, Random r, BooleanPair bp) throws EvalException {
			...
			for (ArrayList<LCONST> sub_inst : possible_subs) {
				...
				Object interm_result = _e.sample(subs, s, r, bp);
				if (DEBUG_EXPR_EVAL)
	...
	public static class PVAR_EXPR extends BOOL_EXPR {
		...
		public Object sample(HashMap<LVAR,LCONST> subs, State s, Random r, BooleanPair bp) throws EvalException {
			...
			PVARIABLE_DEF pvar_def = s._hmPVariables.get(_sName);
			if(pvar_def instanceof RDDL.PVARIABLE_ACTION_DEF)
				bp.a = true;
	...
	public static class IF_EXPR extends EXPR { 
		...
		public Object sample(HashMap<LVAR,LCONST> subs, State s, Random r, BooleanPair bp) throws EvalException {
			Object test = _test.sample(subs, s, r, bp);
			if (!(test instanceof Boolean))
				throw new EvalException("RDDL.IF_EXPR: test " + _test + " did not evaluate to boolean: " + test+ "\n" + this);
			if (((Boolean)test).booleanValue())
				return _trueBranch.sample(subs, s, r, bp);
			else
				return _falseBranch.sample(subs, s, r, bp);
		}

		public EXPR getDist(HashMap<LVAR,LCONST> subs, State s) throws EvalException {
			Object test = _test.sample(subs, s, null, new BooleanPair());
			if (!(test instanceof Boolean))
				throw new EvalException("RDDL.IF_EXPR: test " + _test + " did not evaluate to boolean: " + test+ "\n" + this);
			if (((Boolean)test).booleanValue())
	...
	public static class SWITCH_EXPR extends EXPR {
		...
		public Object sample(HashMap<LVAR,LCONST> subs, State s, Random r, BooleanPair bp) throws EvalException {
			...
			try {
				...
				ENUM_VAL seval = (ENUM_VAL)_enumVar.sample(subs, s, r, bp);
				...
				for (CASE c : _cases)
					if (seval.equals(c._sEnumValue))
						return c._expr.sample(subs, s, r, bp);
		...
		public EXPR getDist(HashMap<LVAR,LCONST> subs, State s) throws EvalException {
			...
			try {
				...
				if (etd == null)
					throw new EvalException("Enumerated variable " + _enumVar._sName + " is not defined.");
				ENUM_VAL seval = (ENUM_VAL)_enumVar.sample(subs, s, null, new BooleanPair());
	...
	public static class QUANT_EXPR extends BOOL_EXPR {
		...
		public Object sample(HashMap<LVAR,LCONST> subs, State s, Random r, BooleanPair bp) throws EvalException {
			...
			for (ArrayList<LCONST> sub_inst : possible_subs) {
				...
				// Update result
				Boolean interm_result = (Boolean)_expr.sample(subs, s, r, bp);
	...
	public static class CONN_EXPR extends BOOL_EXPR {
		...
		public Object sample(HashMap<LVAR,LCONST> subs, State s, Random r, BooleanPair bp) throws EvalException {
			...
			if (_sConn == IMPLY) {
				Boolean b1 = (Boolean)_alSubNodes.get(0).sample(subs, s, r, bp);
				if (!b1)
					return TRUE;
				else
					return (Boolean)_alSubNodes.get(1).sample(subs, s, r, bp);
			} else if (_sConn == EQUIV) {
				return ((Boolean)_alSubNodes.get(0).sample(subs, s, r, bp)).equals(
						(Boolean)_alSubNodes.get(1).sample(subs, s, r, bp));
			}

			// Now handle AND/OR
			Boolean result = null;
			for (BOOL_EXPR b : _alSubNodes) {
				Boolean interm_result = (Boolean)b.sample(subs, s, r, bp);
				if (result == null)
					result = interm_result;
				else 
		...
		public void collectGFluents(HashMap<LVAR, LCONST> subs,	State s, HashSet<Pair> gfluents) 
			...
			if (_sConn == AND || _sConn == OR) {
				for (BOOL_EXPR b : _alSubNodes) {
					if (b instanceof PVAR_EXPR) {
						...
						if (s.getPVariableType(p._sName) == State.NONFLUENT) {
							boolean eval = (Boolean)p.sample(subs, s, null, new BooleanPair());
						...
						else if (_sConn == AND && ACTION_DEPENDENCY && s.getPVariableType(((PVAR_EXPR)b)._sName) == State.ACTION) {
							...
							Boolean eval = (Boolean)b.sample(subs, s, null, new BooleanPair());
				...
				if (s.getPVariableType(p._sName) == State.NONFLUENT) {
					boolean lhs_condition = (Boolean)p.sample(subs, s, null, new BooleanPair());
					if (!lhs_condition)
	...
	public static class NEG_EXPR extends BOOL_EXPR {
		...
		public Object sample(HashMap<LVAR,LCONST> subs, State s, Random r, BooleanPair bp) throws EvalException {
			Boolean b = (Boolean)_subnode.sample(subs, s, r, bp);
			return !b;
		}
